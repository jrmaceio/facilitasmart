...
/**
 * Baixa por Lote  
 * ContasReceberSelectionList Record selection
 * @author  <your name here>
 */
class ContasReceberSelListBxLote2 extends TStandardList
{
    ...
    public function __construct()
    {
        ...
        $situacao->addItems(array(0 => 'Todos', 1 => 'Liquidado', 2 => 'Acordo', 0 => 'Em Aberto' ));
        $situacao->setDefaultOption(FALSE);
        
       
        $dt_liquidacao0 = new TDate('dt_liquidacao0');
        $dt_liquidacao0->setMask('dd/mm/yyyy');
        $dt_liquidacao0->setDatabaseMask('yyyy-mm-dd');
        ...
        $this->form->addFields( [new TLabel('Id')], [$id], [new TLabel('Classe')], [$classe_id] );
        $this->form->addFields( [new TLabel('Unidade')], [$unidade_id], [new TLabel('Situação')], [$situacao]);
        $this->form->addFields( [new TLabel('Mês Ref.')], [$mes_ref], [new TLabel('Data Liquidação')], [$dt_liquidacao0] );
        //$this->form->addFields( [new TLabel('Proprietário')], [$unidade_nome], [$unidade_morador] );
        ...
        $this->form->addAction( 'Limpa',  new TAction(array($this, 'onClear')), 'fa:eraser yellow');
        $this->form->addAction( 'Baixar', new TAction(array($this, 'onConfBaixar')), 'fa:check-circle-o green' );
        $this->form->addAction( 'Estornar', new TAction(array($this, 'onConfEstorno')), 'fa:check-circle-o green' );
        ...
        $column_dt_pagamento = new TDataGridColumn('dt_pagamento', 'Pagamento', 'center');
    ...
    public function onSelecionarDesmarcarTodos($param)
    {
        try
        {
            ...
            if (TSession::getValue('ContasReceberListagem_filter_dt_liquidacao')) {
                $criteria->add(TSession::getValue('ContasReceberListagem_filter_dt_liquidacao')); // add the session filter
            }
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('ContasReceberListagem_filter_dt_liquidacao',   NULL);
        ...
        if (isset($data->dt_liquidacao0) AND ($data->dt_liquidacao0)) {
            $filter = new TFilter('dt_liquidacao', '=', "{$data->dt_liquidacao0}"); // create the filter
            TSession::setValue('ContasReceberListagem_filter_dt_liquidacao',   $filter); // stores the filter in the session
        }
    ...
    public function showResults()
    {
        ...
        if ($selected_objects)
        {
            ...
            foreach ($selected_objects as $selected_object)
            {
                ...
                if (!$unidade) {
                    TTransaction::close();
                    new TMessage('info', 'Problema na seleção de registros, a unidade pode está vazia para lançamento manual!');
                    return;
                }
    ...
    public function onBaixar($param)
    {
              ...
              try
                ...
                { 
                    ...
                    $status = ContasReceber::retornaStatusFechamento($object->condominio_id, 
                                    $dataliquidacao[1].'/'.$dataliquidacao[0],
        ...
        if (number_format($soma_lote, 2, ',', '.') != $param['valor_lote']) {
            new TMessage('info', 'O lote selecionado ('.$contador.' titulos) tem valor divergente do total informado, será considerado o valor em desconto.');
        ...
        if ($selected_objects)
        {
            ...
            foreach ($selected_objects as $selected_object)
            {
                ...
                if ( $object->situacao == '0' )
                {
                    ...
                    if (isset($param['desconto'])) {
                        //var_dump($param['desconto']);
                        //var_dump($conta_titulos);
                    ...
                    //var_dump($status);
                    if ( $status != 0 or $status == ''){
                        new TMessage('info', 'Não existe um fechamento em aberto com o Mês Ref da data baixa ! Operação Cancelada.');
                    }else {
        ...
        if ($selected_objects)
        {
            foreach ($selected_objects as $selected_object)
            {
                try
                {
                    ...
                    if ($selected_object['id'] == null) {
                        new TMessage('error', '<b>Error</b> ' . 'Problema na seleção de registro(s)!'); // shows the exception error message
                        TTransaction::close(); // close the transaction
                    ...
                    if ($object->arquivo_retorno == 'AUTO-M') 
                    {
                        new TMessage('error', '<b>Error</b> ' . 'Título baixado automaticamente, o estorno não é possível.'); // shows the exception error message
                        TTransaction::close(); // close the transaction
                        return;
                     
                    }

                    if ($object->arquivo_retorno == 'WEBHOO') 
                    {
                        new TMessage('error', '<b>Error</b> ' . 'Título baixado automaticamente, o estorno não é possível.'); // shows the exception error message
                        TTransaction::close(); // close the transaction
                        return;
                    ...
                    }else if ( $object->situacao == '1' ) 
                    {
                        ...
                        $status = ContasReceber::retornaStatusFechamento($object->condominio_id, 
                            $databaixa[1].'/'.$databaixa[0], $object->conta_fechamento_id);
